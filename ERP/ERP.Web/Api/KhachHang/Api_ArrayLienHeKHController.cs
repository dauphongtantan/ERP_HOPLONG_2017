...
namespace ERP.Web.Api.KhachHang
{
    public class Api_ArrayLienHeKHController : ApiController
    {
        ...
        public IHttpActionResult PostKH_LIEN_HE(List<LienHeKH> lh)
        {
            if (!ModelState.IsValid)
            {
            ...
            {
                var check = db.KH_LIEN_HE.Where(x => x.SDT1 == item.SDT1).FirstOrDefault();
                if (check == null)
                {
                ...
                {
                    return Ok("Lỗi! Đã có liên hệ này trong hệ thống,xin vui lòng kiểm tra lại");
                }
