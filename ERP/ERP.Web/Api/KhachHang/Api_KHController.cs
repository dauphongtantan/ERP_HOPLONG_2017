...
namespace ERP.Web.Api.HeThong
{
    ...
    public class Api_KHController : ApiController
    {
        ...
        public List<HopLong_TimAllKH_TheoSDT_MST_EMAIL_Result> TimKhachTheoSDT(int page, ThongTinTimKiem timkiem)
        {
            var query = db.Database.SqlQuery<HopLong_TimAllKH_TheoSDT_MST_EMAIL_Result>("HopLong_TimAllKH_TheoSDT_MST_EMAIL @macongty, @tukhoa, @sotrang", new SqlParameter("macongty", timkiem.macongty), new SqlParameter("tukhoa", timkiem.tukhoa), new SqlParameter("sotrang", page));
            var result = query.ToList();
        ...
        public IHttpActionResult ThemMoiKH(KH kH)
        {
            ...
            var KHS = db.KHs.Where(x => x.MST == kH.MST || (x.HOTLINE == kH.HOTLINE && kH.HOTLINE != "") || (x.EMAIL == kH.EMAIL && kH.EMAIL != "")).FirstOrDefault();
            if (KHS == null)
            {
                KH khach = new KH();
                ...
                khach.KHACH_MUA_DOANH_SO_CAO = kH.KHACH_MUA_DOANH_SO_CAO;
                khach.KHACH_DAC_BIET = kH.KHACH_DAC_BIET;
                khach.NGAY_TAO = DateTime.Today.Date;
                db.KHs.Add(khach);
                db.SaveChanges();
                return Ok(khach);
            }
            ...
            {
                return Ok("Đã có khách hàng này trong hệ thông,xin vui lòng không nhập thêm");
            }
