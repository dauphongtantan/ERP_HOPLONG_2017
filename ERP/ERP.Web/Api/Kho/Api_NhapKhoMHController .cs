                ...
                foreach (ChiTietNhapKhoMH item in kho_NhapKhoMH.ChiTiet)
                {
                    ...
                    foreach (var abc in newkhogiu)
                    {
                        if(newItem.SO_LUONG > abc.SL_CAN_GIU)
                        {
                            ...
                            if (newhanggiu == null)
                            {
                                ...
                                newhanggiu.MA_HANG = item.MA_HANG;
                        ...
                        var query3 = db.BH_CT_DON_HANG_PO.Where(x => x.ID == abc.ID_PO_BAN).FirstOrDefault();
                        if(query3 != null)
                        {
                            query3.CAN_GIU_HANG = true;
                            db.SaveChanges();
                        }
                    ...
                    if ((pomh.SL - pomh.SL_DA_VE) >= newItem.SO_LUONG)
                    {
                        pomh.SL_DA_VE += Convert.ToInt32(newItem.SO_LUONG);
                            ...
                            var danhapkho = db.MH_PO_CT_MUA_HANG.Where(x => x.MA_SO_PO == pomh.MA_SO_PO && (x.SL - x.SL_DA_VE > 0)).ToList().Count();
                            if (danhapkho == 0)
                            {
                                var query2 = db.MH_PO_MUA_HANG.Where(x => x.MA_SO_PO == pomh.MA_SO_PO).FirstOrDefault();
                                if (query2 != null)
                                {
                                    query2.DA_NHAP_KHO = true;
                                    query2.MA_SO_NHAP_KHO = nk.SO_CHUNG_TU;
                                    db.SaveChanges();
                                }
                            }
