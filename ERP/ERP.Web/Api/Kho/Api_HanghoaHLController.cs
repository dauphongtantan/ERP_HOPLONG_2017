...
namespace ERP.Web.Areas.HopLong.Api.Kho
{
    public class Api_HanghoaHLController : ApiController
    {
        ...
        public IHttpActionResult PostHH(HH Hh)
        {
            ...
            hanghoa.NGAY_TAO = DateTime.Today.Date;
            db.HHs.Add(hanghoa);
        ...
        // PUT: api/Api_HanghoaHL/5
        [Route("api/Api_HanghoaHL/SuaMoTa/{id}")]
        public IHttpActionResult SuaMoTa(string id,HH hanghoa)
        {
            var query = db.HHs.Where(x => x.MA_HANG == id).FirstOrDefault();
            if(query != null)
            {
                query.TEN_HANG = hanghoa.TEN_HANG;
                db.SaveChanges();
            }
